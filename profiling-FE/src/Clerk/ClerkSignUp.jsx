...
const ClerkSignUp = () => {
    ...
    const { firstName, lastName, userName, email, password } = clerk
    ...
    const onSubmit = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.post("http://localhost:8080/api/clerk/createClerkAccount", clerk)
            if (response.status == 200) {
                alert(" CLERK ACCOUNT CREATED ")
                navigate("/clerkSignIn")
                setFormData(initialFormData);
            }
        } catch (error) {
            alert(" CLERK ACCOUNT ALREADY EXISTED ")
        }
    };
            ...
            <div className='container mt-5 py-5' style={{
                ...
                <>
                    <form onSubmit={(e) => onSubmit(e)}>
                        ...
                        <Link to='/clerkSignIn' onClick={loadContent}> Go Back </Link>
